...
// Select candidates by order
point_set_t* select_candidates(point_set_t *p, int c);
...
inline double duration(double start){
// calcule la durée en seconde entre l'instant 'start' et maintenant
    //return (clock()-start)*(1000.0/CLOCKS_PER_SEC);
    return (omp_get_wtime()-start)*(1.0);
}
